...
- (id)initWithToken:(CLToken *)token andDelegate:(NSObject<CLTokenViewDelegate>*)delegate
{
	...
	if (self) {
		...
		if([self.delegate respondsToSelector:@selector(textColorForTokenViewWithToken:)]){
			self.label.textColor = [self.delegate textColorForTokenViewWithToken:token];
		}
        ...
        [attrString setAttributes:@{NSForegroundColorAttributeName : self.label.textColor}
                            range:tintRange];
...
- (void)reload{
	if([self.delegate respondsToSelector:@selector(textColorForTokenViewWithToken:)]){
		self.label.textColor = [self.delegate textColorForTokenViewWithToken:self.token];
	}
}
