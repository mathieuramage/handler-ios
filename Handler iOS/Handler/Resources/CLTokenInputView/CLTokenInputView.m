...
static CGFloat const PADDING_TOP = 2.5;
static CGFloat const PADDING_BOTTOM = 2.5;
static CGFloat const PADDING_LEFT = 8.0;
static CGFloat const PADDING_RIGHT = 16.0;
static CGFloat const STANDARD_ROW_HEIGHT = 20.0;
...
- (void)addToken:(CLToken *)token
{
    ...
    CLTokenView *tokenView = [[CLTokenView alloc] initWithToken:token andDelegate:self];
	...
	if ([self.delegate respondsToSelector:@selector(startValidationWithString:)]){
		[self.delegate startValidationWithString:self.textField.text];
	}
}
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string{
	...
	self.validationTimer = [NSTimer timerWithTimeInterval:0.3 target:self selector:@selector(validateCurrentInput) userInfo:nil repeats:NO];
	[[NSRunLoop mainRunLoop] addTimer:self.validationTimer forMode:NSRunLoopCommonModes];
	return YES;
}
...
- (UIColor *)textColorForTokenViewWithToken:(CLToken *)token{
	if([self.delegate respondsToSelector:@selector(textColorForTokenViewWithToken:)]){
		return [self.delegate textColorForTokenViewWithToken:token];
	}
