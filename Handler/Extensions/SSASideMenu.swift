...
extension UIViewController {
	...
	@IBAction func presentLeftMenuViewController() {
		...
		AppAnalytics.fireContentViewEvent(contentId: MenuEvents.viewMenu, event: MenuEvents)
	}
...
class SSASideMenu: UIViewController, UIGestureRecognizerDelegate {
	...
	fileprivate func animateMenuViewController(_ side: SSASideMenuSide) {
		...
		if side == .left {
			...
			contentViewContainer.center = CGPoint(x: centerX+contentViewContainer.bounds.size.width / 2, y: contentViewContainer.center.y)
	...
	internal var leftMenuRightInset: CGFloat = 30.0
