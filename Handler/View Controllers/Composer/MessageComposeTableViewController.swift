...
//                subjectTextField.text = "\(message.replyPrefix) \(message.subject ?? "")"
    ...
    func send() {
		...
		func checkSendDismiss() {
			...
			} else if let replyTo = self.messageToReplyTo, let id = replyTo.conversation?.identifier {
				MessageManager.replyToConversation(conversationId: id, content: message, subject: subject, recipients: recipients, callback: { message in
					self.navigationController?.dismiss(animated: true, completion: nil)
				})
